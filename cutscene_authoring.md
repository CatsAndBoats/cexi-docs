...
`cexi.cutscene.v1` JSON "def", and compiled natively to FFXI event bytecode + a camera scene
...
Editor timeline (state.tracks)
  → buildCutscene() → cexi.cutscene.v1 def  (saved to cutscene-defs/<zone>_<event>.json)
  → compile_cutscene()  [src/cexi/event/xi_compile.py]
...
## Backend — the compiler (`src/cexi/event/xi_compile.py`)
...
### Bridge endpoints (`src/cexi/zone/xi_bridge.py`)
...
| Area | File |
|---|---|
| Compiler | `src/cexi/event/xi_compile.py` |
| Event decode / dialogue author | `src/cexi/event/xi_event.py`, `xi_author.py`, `xi_commands.py` |
| Bridge endpoints | `src/cexi/zone/xi_bridge.py` |
