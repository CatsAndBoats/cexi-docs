...
```bash
uv run cexi mesh import <dat> [model]
uv run cexi mesh import ROM/7/97             # uses the exported model automatically
uv run cexi mesh import ROM/7/97 model.fbx   # or an explicit model
```
...
- Animation is not part of this workflow (use `cexi anim`); the skeleton and
