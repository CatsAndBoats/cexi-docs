# `cexi fx` — visual effects in a DAT

Inspect and edit the **visual effects** baked into a DAT: fountain spray, lamp
glow, fire, bubbles, clouds, etc. Core commands:

```
cexi fx json   <dat>                 # dump all effects + decoded params to JSON
cexi fx delete <dat> <name>...       # remove effect(s) by name or name-prefix
cexi fx set    <dat> <name>... ...   # edit params (pos/scale/color/draw-distance)
cexi fx copy   <dat> <src> [--from/--at/--at-pos/--offset/--name]  # duplicate (cross-DAT too)
cexi fx export <dat> <effect>        # export its 3D mesh + material/texture + JSON bundle
```

`<dat>` is a path or a ROM-relative spec like `ROM/1/41`. Implemented in
the `src/cexi/fx/` package (`xi_core.py` shared; `xi_list.py`/`xi_dump.py`/`xi_delete.py`/`xi_set.py`/`xi_copy.py`/`xi_export.py` per command). For the byte-level effect format see
...
- The meshes an effect places are often **orphan `0x2E` meshes** with no `0x1C`
  ...
  `cexi zone export`** (this is why effect geometry like the fountain puddle never
...
(sec3/4) — and is always included in `cexi fx export`'s per-effect JSON.

---

## `cexi fx delete <dat> <name>...`

Removes one or more effects and rebuilds the DAT.

```sh
uv run cexi fx delete ROM/1/41 grid              # one effect (the puddle)
uv run cexi fx delete ROM/1/41 tki awa grid      # prefixes: tki1-5 + awa1-6 + grid
uv run cexi fx delete ROM/1/41 tki --dry-run     # preview, write nothing
```
...
- A pristine **`<dat>.base`** backup is created (shared with `cexi zone import`).
  Restore the whole DAT from it to undo.

---

## `cexi fx set <dat> <name>... [params]`
...
uv run cexi fx set ROM/1/41 tki --color 00FF00 --scale-mul 4 --range 100 500

# move a single effect
uv run cexi fx set ROM/1/41 grid --pos -16 -0.4 5
```

| flag | effect | how it's found |
|------|--------|----------------|
| `--pos X Y Z` (alias `--at-pos`) | local position (`--at-pos` matches `/cexi pos`) | 3×f32 after the placed-mesh/texture ref |
...
```sh
# same-DAT
uv run cexi fx copy ROM/1/41 tki5 --offset 6 0 0          # 6th fountain jet
# cross-DAT: a Castle Zvahl wall torch onto the fountain (brings fire texture + light sub-resources)
uv run cexi fx copy ROM/1/41 lb09 --from ROM/0/60 --at-pos -20.7 -2 5.2 --name fir0
# place at an existing object's spot, nudged up 2
uv run cexi fx copy ROM/1/41 lb09 --from ROM/0/60 --at funsui --offset 0 -2 0 --name fir0
```
...
| `--pos X Y Z` / `--at-pos X Y Z` | absolute position (`--at-pos` matches the `/cexi pos` output) |
| `--at REF` | place at an existing **effect** (FourCC) or **placed object** (mesh id) |
| `--offset DX DY DZ` | nudge from the source / `--at` / `--pos` |

- **`/cexi pos`** in-game (the `cexitools` Ashita addon, `addon/cexitools/`) prints a
  ready `--at-pos X Y Z` line and a full `cexi fx copy … --at-pos …` command — stand
...
```sh
uv run cexi fx export ROM/1/41 tki5
...
on top of mesh-merge/placement edits. But `cexi zone import` **rebuilds from
