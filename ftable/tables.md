...
[`cexi ftable info`](info.md) instead.
...
```
uv run cexi ftable tables          # readable table
uv run cexi ftable tables --json   # machine-readable, includes the gear-region split
```
...
> context by [`cexi ftable info`](info.md).
