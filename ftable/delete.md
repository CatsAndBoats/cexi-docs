...
[`cexi ftable set`](set.md). Use it to unregister a custom mount, entity, or any
custom-ROM file_id.
...
```
uv run cexi ftable delete --file-id N [N ...]
uv run cexi ftable delete --modelid N [N ...]
uv run cexi ftable delete --file-id N --dry-run
```
...
```
uv run cexi ftable delete --modelid 15000 --dry-run
```
...
Run `cexi model json --free` to confirm the slot is free and see the updated
slot count:
