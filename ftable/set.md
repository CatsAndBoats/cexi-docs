...
client-side file id outside the entity/gear bands that `cexi dats build` manages for you.
...
```
uv run cexi ftable set --file-id N  --rom R --subdir D --file F
uv run cexi ftable set --modelid N  --rom R --subdir D --file F
uv run cexi ftable set ... --no-base
uv run cexi ftable set ... --dry-run
```
...
[`cexi dats`](../dats/README.md) instead.
...
```
uv run cexi ftable set --file-id 102755 --rom 10 --subdir 10 --file 1
```
...
Run "cexi ftable expand" to grow the tables first.
```
...
`cexi ftable set` and [`cexi ftable delete`](delete.md) are symmetric — `delete` zeros the
same file_id in **both** the base and `ROM{N}` tables:

```
uv run cexi ftable delete --file-id 102755
```

Edits go through the output mirror with a `.base` backup, so `cexi ftable reset` can
