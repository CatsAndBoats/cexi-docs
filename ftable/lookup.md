...
```
uv run cexi ftable lookup --file-id N                 # base FTABLE.DAT (default)
uv run cexi ftable lookup --table 10 --file-id N      # ROM10/FTABLE10.DAT
uv run cexi ftable lookup <ftable_file> --file-id N   # explicit path (overrides --table)
uv run cexi ftable lookup --modelid N
```
...
```
uv run cexi ftable lookup FTABLE.DAT --file-id 1608
```
...
```
uv run cexi ftable lookup ROM10\FTABLE10.DAT --modelid 15000
```
...
```
uv run cexi ftable lookup FTABLE.DAT --file-id 113239
```
