...
> **TL;DR**
> ```
> uv run cexi ftable expand     # run once — sets up BOTH entity + gear
> uv run cexi ftable info       # see the ranges + what's free
> ```
...
```
uv run cexi ftable info       # ranges, what's used, recommended starts
uv run cexi ftable tables     # raw per-ROM table sizes
```
...
`cexi ftable expand` is the all-in-one. Two focused subcommands exist for doing
just one side:

| Command | What it does |
|---|---|
| `uv run cexi ftable expand` | **Both** — grows the tables and wires up gear. The normal choice. |
| `uv run cexi ftable expand --no-gear` | Entity buffer only (skips gear setup + its `FFXiMain.dll` check). |
| `uv run cexi ftable expand entity [N]` | Entity buffer only, up to an explicit modelid `N`. |
| `uv run cexi ftable expand gear [N]` | Gear windows only, up to an explicit per-slot max `N`. |
...
- the **pivot / override pack** — `FFXI_PIVOT_DIR` (e.g. Ashita's
  `…/polplugins/DATs/catseyexi/`), which ships its own `FTABLE.DAT` +
...
> the usual cause. `cexi ftable tables` shows every size; re-run `cexi ftable
> expand` (with `--pivot`) to bring them back in line, or `cexi ftable reset`.
...
Your actual custom models get registered later by `cexi dats build`.

---

## Config — one source of truth

The ranges live in `src/cexi/xi_config.py` (override any with an env var):

| Setting | Default | Env var | Meaning |
|---|---|---|---|
| `MAX_ENTITY_MODELID` | `30000` | `CEXI_MAX_ENTITY_MODELID` | Entity ceiling — top of the entity buffer |
| `MAX_GEAR_MODELID` | `4095` | `CEXI_MAX_GEAR_MODELID` | Gear ceiling per race + slot (12-bit max) |
| `GEAR_RECOMMENDED_START` | `3000` | `CEXI_GEAR_RECOMMENDED_START` | Suggested first custom gear modelid |
...
> `cexi ftable reset`, then `cexi ftable expand` again.
...
- [info.md](info.md) — `cexi ftable info`: the range / layout dump (start here to see what's free)
