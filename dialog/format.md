...
`cexi event dialogue` decodes these — see [export.md](export.md).
...
`cexi event dialogue`'s `text` field is the **readable** rendering: normal characters,
...
`cexi event dialogue edit` re-encodes custom text back into this format — see
...
`src/cexi/dialog/_sjis_data.py`, **generated** from Shining Fantasia's
