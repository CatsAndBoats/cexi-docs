# `cexi event dialogue` — decode dialog DATs to JSON
...
```
cexi event dialogue info <DAT>            # entry count + opcode histogram, no export
cexi event dialogue search <DAT> "text"   # find the index + entry for some text
cexi event dialogue export <DAT>          # writes 3 sibling JSON files under exports/event/dialogue/<rom>/
```

## Finding an entry (`search`)

To find the index of a line (e.g. to pass to `cexi event dialogue edit --index`), search
by text. Matching is case-insensitive substring by default and flattens line
breaks, so a query can span them:

```bash
cexi event dialogue search ROM/25/39.DAT "Mog House"
cexi event dialogue search "Lower Jeuno" "when I was but a lass"  # by zone name
cexi event dialogue search 245 "fountain"                        # by zone id
cexi event dialogue search ROM/1/41 "fountain"                   # a zone model DAT also routes
cexi event dialogue search ROM/25/39.DAT "lies to the north"     # matches across a line break
cexi event dialogue search ROM/25/39.DAT "San d.Oria lies" --regex
```
...
mirroring the ROM path like the other `cexi` export commands. They split the data
...
```bash
cexi event dialogue info ROM/25/39.DAT                       # Southern San d'Oria summary
cexi event dialogue export ROM/25/39.DAT                       # → 39.json + 39.opcodes.json + 39.hex.json
cexi event dialogue export ROM/25/39.DAT --no-raw              # skip the big hex file
cexi event dialogue export ROM/25/39.DAT --grep "Mog House" --preview
cexi event dialogue export ROM/25/39.DAT -o sandoria.json      # explicit base path
cexi event dialogue export ROM/25/39.DAT --index 7663 --json | jq   # one combined doc to stdout
```
...
- Decoding is read-only. To author custom dialog, see `cexi event dialogue edit`
