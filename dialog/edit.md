# `cexi event dialogue edit` — author custom dialog
...
```
cexi event dialogue edit <DAT> --index <N> --text "<text>"   [--dry-run]
```

- Find the `--index` with `cexi event dialogue search <DAT> "some words"`.
...
```bash
cexi event dialogue edit ROM/25/39.DAT --index 4 --text "ho ho ho"
cexi event dialogue edit ROM/25/39.DAT --index 0 --text "Welcome home!\nYour Mog House awaits.\v"
cexi event dialogue edit ROM/25/39.DAT --index 5 --text "Hello {player}!\nThe shop opens shortly.\n{auto:5}"
cexi event dialogue edit ROM/25/39.DAT --index 0 --text "New line here." --dry-run
```
...
- The prompt code `\v` is flow control, so it shows up in the `--text` round-trip
  ...
  for a keypress.
- **Undo:** `cexi event dialogue reset <DAT>` puts the DAT back to pristine — it
