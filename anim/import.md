...
uv run cexi anim import ROM/217/32 idl
uv run cexi anim import ROM/218/91 idl --no-base      # after a mesh import
uv run cexi anim import rom/37/13 poi                 # emote, all slots (see emotes.md)

# Create a NEW track from a full-body glTF (both forms equivalent)
uv run cexi anim import ROM9/25/40 tlk yap.gltf
uv run cexi anim import ROM9/25/40 --add tlk yap.gltf

# Import a partial clip cleanly (only the bones the glTF drives; rest held still)
uv run cexi anim import ROM9/25/40 tlk yap.gltf --static-base

# LAYER a clip onto another: talk (bones 7 & 9) over the live idle, as a new track
uv run cexi anim import ROM9/25/40 --anim idl --add tlk \
...
```bash
uv run cexi anim import ROM9/25/40 tlk yap.gltf          # positional new name + glTF
uv run cexi anim import ROM9/25/40 --add tlk yap.gltf    # same, explicit
uv run cexi anim export ROM9/25/40 --anim tlk            # confirm it round-trips
```
...
```bash
uv run cexi mesh import ROM/x/y            # restores baseline, writes the mesh
uv run cexi anim import ROM/x/y idl --no-base   # writes the animation, keeps the mesh
```
