...
```bash
uv run cexi anim export <dat> [--anim NAME] [--fbx] [--output DIR]
                                       [--mesh [LOOK]] [--race NAME] [--skeleton-dat PATH]
uv run cexi anim export ROM/217/32 --anim idl
uv run cexi anim export rom/37/13 --anim poi --fbx        # emote (see emotes.md)
```
...
```
uv run cexi anim export rom/37/13 --anim poi
