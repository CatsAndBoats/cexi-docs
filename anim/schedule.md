...
```bash
uv run cexi anim schedule add    <dat> --clip tlk0 [--tag NAME] [--loop/--no-loop] [--loops N]
                                       [--dur N] [--blend N] [--trans-in N] [--trans-out N]
uv run cexi anim schedule create <dat> [--tag NAME] [--clip A --clip B …]
                                       [--loop-last/--no-loop-last] [--blend N]
uv run cexi anim schedule edit   <dat> <tag> [--blend N] [--trans-in N] [--trans-out N]
                                       [--loop/--no-loop] [--loops N] [--dur N]
uv run cexi anim schedule copy   <dat> <src_tag> <dst_tag>
uv run cexi anim schedule list   <dat>
```
...
```bash
uv run cexi anim schedule create rom9/25/40                          # interactive wizard
uv run cexi anim schedule create rom9/25/40 --tag sit0 --clip sitd --clip siti
```
...
```bash
uv run cexi anim schedule edit rom9/25/40 tlk0 --blend 5          # snappier crossfade
uv run cexi anim schedule edit rom9/25/40 tlk0 --trans-out 20     # keep in, slow release
uv run cexi anim schedule edit rom9/25/40 tlk0 --no-loop --dur 60
uv run cexi anim schedule edit rom9/25/40 tlk0 --loops 28         # play 28× then hold
```
...
```bash
uv run cexi anim schedule copy rom9/25/40 tlk0 tlk1
uv run cexi anim schedule edit rom9/25/40 tlk1 --loops 1 --blend 8
```
...
```bash
uv run cexi anim import <dat> tlk yap.gltf --static-base --add-schedule --blend 12
uv run cexi anim import <dat> --anim idl --add tlk --layer yap.gltf \
