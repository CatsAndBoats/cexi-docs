...
entirely within cexi-tools, via our own native Recast/Detour library (no external
...
cexi produces it with this stack:
...
This produces `cexi_navmesh.dll` (Windows) / `.so` (Linux) under `build/`.
`cexi zone navmesh` finds it automatically (`build/Release/`, `build/Debug/`, or
...
(On CatsEyeXI, `catseyexi/navmeshes` is symlinked to `xiNavmeshes/`.) **Back up
...
```bash
cexi zone navmesh-info exports/zone/rom/1/41/41.nav        # ours
cexi zone navmesh-info /path/to/navmeshes/Lower_Jeuno.nav  # stock
cexi zone navmesh-info <nav> --tiles                       # per-tile breakdown
```
...
The server loader is `catseyexi/src/map/navmesh.cpp` (`CNavMesh::load`). See
