...
Reverse-engineered while building `cexi zone export`, verified by ripping Lower
...
xim reference (`name.startsWith("_") → discardThreshold 0.375`), and cexi's own editor
...
- **Normals MUST be unit length.** The client does **not** renormalize, so `dot(N, light)`
  ...
  *port* renormalizes in its vertex shader, so a bad mesh still looks correct **in the
  editor** — only the real client exposes it. cexi's importer and encoder now force unit
...
- Retail meshes keep vColor near neutral and let the engine light them; a few bake mild
  ambient occlusion into vColor for crease/contact shadow. cexi can bake AO too, but it's
