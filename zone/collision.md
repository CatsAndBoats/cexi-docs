...
```bash
uv run cexi zone export ROM/1/41 --collision
```
...
4. Run `cexi zone import ROM/1/41 --add-collision blocker.obj`.
...
`cexi zone reset ROM/1/41` to start clean.
...
```bash
cexi zone import ROM/1/41 --add-collision blocker.obj --scale 0.1   # coords came out 10x too large
```
...
see **[navmesh.md](navmesh.md)** (`cexi zone navmesh <dat>`). It's a separate
