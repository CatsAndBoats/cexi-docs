...
a zone DAT (`ROM/<dir>/<dat>.DAT`). Mirrors the `cexi zone` command group.
...
`swap-placement`, `delete`. The old `cexi zone object` group remains as a hidden
compatibility alias.
...
uv run cexi gui zone                       # move/rotate/scale, copy/paste, delete
#   → Changes ▸ Export JSON ▸ zone-changes.json
uv run cexi zone import-json zone-changes.json

# Add a new prop from a model
uv run cexi object export ROM/1/41 gaitou01     # get a GLB to base it on
uv run cexi object import ROM/1/41 gaitou01.glb --name lamp --pos -8.8 0 -7.7 --rot 3.142 1.044 3.142

# Export collision mesh + edit + append blockers
uv run cexi zone export ROM/1/41 --collision  # <stem>.collision.obj overlays the zone .glb
# author new blocker geometry in DCC (same (-x,-y,z) frame), export as blocker.obj
uv run cexi zone import ROM/1/41 --add-collision blocker.obj

# Bake a server navmesh from the (edited) collision, then validate it
uv run cexi zone navmesh ROM/1/41             # -> exports/zone/rom/1/41/41.nav
uv run cexi zone navmesh-info exports/zone/rom/1/41/41.nav
#   copy to the server as navmeshes/<ZoneName>.nav (e.g. Lower_Jeuno.nav)

# Start over
uv run cexi zone reset ROM/1/41
```
