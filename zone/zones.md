...
`cexi` can generate the current client-backed zone list directly from `ROM/165/84.DAT`
and the merged FTABLE/VTABLE data:

```bash
uv run cexi zone json
```
...
```bash
uv run cexi zone json -o web/leveleditor/zones.json
```
