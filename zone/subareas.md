...
it is a placeholder for (`0` = not a placeholder). cexi decodes it as
`file_id_link` in [`xi_objects.py`](../../src/cexi/zone/xi_objects.py) (`OFF_FILE`).
...
(cexi `scan_file_ids`, [`ftable/xi_core.py`](../../src/cexi/ftable/xi_core.py)).
...
| Resolve sub-area ids → interior DAT paths via FTABLE | backend `zone.subareas` (`src/cexi/zone/xi_bridge.py` `_subareas`) |
