...
```bash
uv run cexi zone export <dat> [--fbx] [--no-sky] [--no-vfx] [--objects] [--collision] [--base] [--raw] [--alpha-scale N]
uv run cexi zone export ROM/1/41            # Lower Jeuno
```
...
uv run cexi zone export ROM/1/41 --objects --fbx --no-sky --no-vfx
