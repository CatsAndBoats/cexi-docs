...
```
uv run cexi zone import-json <changes_json> [dat_path] [--dry-run]
```
...
web editor's VFX panel and in `cexi fx json`).
...
| Op | Behaviour |
|---|---|
| `modify` | Decrypt → patch the full TRS at fixed offsets → re-home in the nearest space-tree leaf → re-encrypt. The single-object CLI equivalent is **`cexi object set-placement <dat> <name> --pos … --rot … --scale …`** |
...
> [`cexi object import`](../object/import.md) but driven from the editor's JSON.
...
|---|---|
| placement move/rotate/scale | `cexi object set-placement <dat> <name> --pos … --rot … --scale …` |
| placement duplicate (copy/paste) | `cexi object clone <dat> <name> --pos …` (rot/scale via a follow-up `set-placement`) |
| placement delete | `cexi object delete <dat> <name>` |
| VFX | `cexi fx set` / `cexi fx delete` / `cexi fx copy` |

---

## Related commands

- **`cexi object delete`** — quick single-object deletion by name
- **`cexi fx delete`** — remove individual VFX by FourCC
- **`cexi fx set`** — modify VFX position / scale / color individually
- **`cexi fx copy`** — duplicate / transplant a VFX
- **`cexi gui zone`** — start the web editor that generates the JSON
