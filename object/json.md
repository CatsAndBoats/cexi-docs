...
```
uv run cexi object json <dat> [--filter TEXT] [--objects-only] [--output PATH]
```
...
uv run cexi object json ROM/1/41 --filter hasi

# small decorative props only
uv run cexi object json ROM/1/41 --objects-only
```
...
- Blanked records (mesh_id zeroed by `cexi object delete`) are omitted.
- Index is the record position in the `0x1C` section — pass it to
  `cexi object swap-placement`. (`import-json` / `set-placement` instead match by
...
- **`cexi zone json --fx`** — list VFX generators in the same zone
- **`cexi object delete`** — blank a placement by name
- **`cexi zone import-json`** — batch move/delete placements from the web editor
