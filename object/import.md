...
```
uv run cexi object import <dat> <source> [--name N] [--pos X Y Z]
...
| `--name N` | Mesh name in the DAT (default: source filename stem). Auto-prefixed `cexi_` and auto-numbered if taken (`cexi_gaitou0101`, `0102`, …) |
...
uv run cexi object import ROM/1/41 exports/object/rom/1/40/obj01/obj01.glb

# add + place an upright street lamp
uv run cexi object import ROM/1/41 exports/object/rom/1/41/gaitou01/gaitou01.glb \
...
it out if it ever causes trouble. All live in `src/cexi/zone/xi_zonedef.py` (helpers) and
...
- **`swap-placement` consumes the overwritten object.** The original object at that slot
  is gone. Pick a throwaway index (`cexi object json`), and remember `reset`
  restores everything.
...
- **`cexi object export`** — pull a mesh out to GLB to edit, then re-import it
- **`cexi zone import-json`** — batch placement edits (move/rotate/scale/add/delete) from the web editor
- **`cexi object json`** — list every placement + index
- **`cexi zone reset`** — restore the DAT to pristine
