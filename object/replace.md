...
Unlike `cexi zone import --rebuild` (which patches vertex positions in-place and
...
```
uv run cexi object replace <dat> <mesh_name> <glb_path>
```
...
uv run cexi object replace ROM/1/41 block03 block03_edited.glb
```
...
changes. Use `cexi tex import` to swap texture images separately.
...
- Does not rebuild collision or sound data. For fully modifying the zone structure
  consider exporting the whole zone and re-importing with `cexi zone import`.

---

## Related commands

- **`cexi object export`** — export the mesh you want to edit
- **`cexi zone export`** — full zone export (all meshes + placements)
- **`cexi zone import`** — import placements + optional vertex patch from a full zone GLB
- **`cexi tex import`** — re-import edited textures separately
