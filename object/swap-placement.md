...
```
uv run cexi object swap-placement <dat> <index> <mesh_name> --pos X Y Z [--rot RX RY RZ] [--scale SX SY SZ]
```
...
uv run cexi object swap-placement ROM/1/41 600 gaitou01 --pos -18.96 0 -3.74 --rot 3.142 1.044 3.142
```
...
- Layers onto prior edits and writes the DAT in place; `cexi zone reset` restores
  the original.
