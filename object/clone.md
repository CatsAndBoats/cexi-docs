...
```
uv run cexi object clone <dat> <mesh_id> --pos X Y Z [--count N]
```
...
uv run cexi object clone ROM/1/41 gaitou01 --pos -5 0 -5
```
...
- Layers onto prior edits; **re-running stacks another copy**. Use `cexi zone reset` to
  start from pristine.
