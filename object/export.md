...
```
uv run cexi object export <dat> <mesh_name> [--fbx] [--output DIR]
```
...
uv run cexi object export ROM/1/41 block03

# export with FBX for C4D
uv run cexi object export ROM/1/41 hasi --fbx

# export to a custom location
uv run cexi object export ROM/1/41 block03 --output /tmp/block03
```
...
2. Writes a GLB with the same `ffxi_root_correction` node as a full `cexi zone export`.
...
tool in the correct orientation and can be re-imported with `cexi object replace`
...
uv run cexi object json ROM/1/41

# or the web level editor — the Objects panel shows every mesh name
uv run cexi gui zone
```

(`cexi zone json` lists the *zones* and their DAT paths.)
...
- **`cexi object replace`** — import an edited GLB back as the mesh geometry
- **`cexi zone export`** — export the whole zone (all meshes + placements)
- **`cexi object delete`** — blank a placement so it stops rendering
