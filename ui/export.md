...
- **Colour is passed through unchanged.** An editor re-saves the PNG with `sRGB`
  ...
  and de-gammas — which made re-imported textures come back visibly darker. cexi
  now always passes `-srgb` so both sides are tagged sRGB and the conversions
  cancel. Nothing to configure; `--gamma-convert` on `cexi utils png2dds` opts
...
```
D:\cexi\catseyexi-client\Ashita\polplugins\DATs\catseyexi
```
...
```cmd
:: cmd / cmder
set FFXI_DIR=D:\cexi\catseyexi-client\Ashita\polplugins\DATs\catseyexi
```
```powershell
# PowerShell
