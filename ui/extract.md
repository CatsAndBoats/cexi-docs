...
```
uv run cexi ui extract <DAT_FILE> [OPTIONS]
```

Shortcut wrapper:

```
uv run cexi ui simple-extract <DAT_FILE>
uv run cexi ui sx <DAT_FILE>
```
...
uv run cexi ui extract ROM/119/50.DAT --list
uv run cexi ui extract ROM/119/51.DAT --list

# extract all textures to exports/ui/50/
uv run cexi ui extract ROM/119/50.DAT

# extract with JSON manifest
uv run cexi ui extract ROM/119/51.DAT --json

# extract to a custom directory
uv run cexi ui extract ROM/0/2.DAT --output-dir exports/ui/lobby
