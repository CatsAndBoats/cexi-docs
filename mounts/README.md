...
Everything about FFXI mounts in cexi — how the client resolves a mount to a model,
the data that makes one appear and be ridable, and the `cexi mount` commands for
...
> Status: **shipped.** The model side uses the [`cexi ftable set`/`delete`](../ftable/set.md)
> primitives; the string side uses a byte-exact `d_msg` codec (`cexi/common/xi_dmsg.py`).
...
|---|---|
| `cexi mount json` | Enumerate ids 0–63: id, EN/JP name, key-item id, file-id, ROM DAT, occupied/free. `--all` for 0–255. |
...
```powershell
cexi dats build dats/update.json --only mount.cyakko
```
...
The original hidden `cexi mount inject` wrote missing key-item blocks by appending them to the end
