...
`cexi mount` commands and the DAT-path quick reference see [README.md](README.md).
...
1. Build the mount model `.DAT`; place it in ROM10 (e.g. `ROM10/50/0.DAT`) and register it:
   `cexi ftable set --file-id <0x019131+id> --rom 10 --subdir … --file …`.
...
`cexi dats build` performs steps 1–3 for mount actions, including Mounts-section key-item placement,
...
- **Client patch via `cexidats`** (an Ashita v4 plugin that already memory-patches `FFXiMain.dll`):
  ...
  **RE lead (atom0s XiPackets 0x00AE):** the client copies `MountDataTbl` into
  ...
  `MountSys` / `MountDataTbl` / `pGlobalNowZone` in the `ffximain_unpacker.py` → Ghidra output
...
- **cexidats** (`D:\cexi-server\cexidats`) — Ashita v4 C++ plugin; runtime memory-patches
...
- **cpp-patches** (`D:\cexi-server\cexi-modules\cpp-patches`) — server-side git-diff patches applied
