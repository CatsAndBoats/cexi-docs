...
Decoder lives in [`src/cexi/audio/xi_core.py`](../../src/cexi/audio/xi_core.py).
...
A Sony MDCT codec — not hand-decodable. cexi shells out to `vgmstream-cli` (with
`-i`, a single linear pass to match the native single-pass output). Auto-detected
from `$CEXI_VGMSTREAM`, `PATH`, or a known install path; `--native-only` skips it.
...
`0x00`). These are an encrypted variant that **vgmstream rejects too** — cexi skips
...
- [README.md](README.md) — the `cexi audio` commands
