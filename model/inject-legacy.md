# cexi entity inject (legacy)

`cexi entity inject` is now a hidden compatibility command. New distributable
content should be authored through `cexi dats prepare` + `cexi dats build`, so
new IDs and ROM paths are recorded in `dats/locks.json` and can be rebuilt from Git.
...
**Prerequisite:** `cexi ftable expand` must be run first. The command will
...
Use `cexi model json --free` to get the blob for the next free slot.
...
```
uv run cexi entity inject <dat_file> [options]
```
...
```
uv run cexi entity inject my_model.DAT --pet-name "MyMonster"
```
...
```
uv run cexi entity inject "ROM10\1\0.DAT" --modelid 15000 --pet-name "TestTiger" --register-existing
```
...
```
uv run cexi entity inject my_model.DAT --dry-run
```
...
1. **Import the SQL** into your LSB database:
   ```
   patches/cexi_monster_inject_<modelid>_<file_id>.sql
   ```
...
Paths are read from environment variables (set in `src/cexi/config.py`):

```
FFXI_DIR        default: D:\cexi\catseyexi-client\Game\FINAL FANTASY XI
CEXI_TOOLS_DIR  default: D:\cexi-tools  (patches/ and backups/ live here)
```
