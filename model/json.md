...
The old `cexi entity list --json` command is still available as a hidden compatibility alias.
...
- **`cexi model json --free`** — just the next free custom slot, no full scan
- **`cexi dats build`** — register new packaged content through a manifest
- **`cexi ftable lookup`** — look up a single file_id or modelid
