...
then uses the same pipeline as `cexi mesh export` — the binary format is
...
```
uv run cexi gear export <race> <slot> <model_id> [--fbx] [--output DIR]
```
...
| `--output DIR` | Output directory (default: `exports/gear/rom/<sub>/<file>/`, mirroring the DAT path like every other cexi export) |
...
uv run cexi gear export Mithra hands 5 --no-fbx

# export Galka main-hand weapon model 1 to a custom location
uv run cexi gear export Galka main 1 --output /tmp/galka_sword
```
...
exports/gear/HumeMale/body/0/
  ...
  *_schema.json      ← `cexi dats prepare`-ready mesh action that re-injects the
                       (edited) GLB back into this gear DAT
```

The `*_schema.json` is emitted by default (like `cexi mesh export`); set the
`SCHEMA_GENERATION=0` env var to suppress it. It is a `cexi.mesh.v1` action with
`source`/`target` pointing at the gear DAT and no `model_id`, so a `dats build`
rebuilds the geometry in place — the same round-trip as `cexi gear import`.

---

## Finding model IDs

Use `cexi gear json` to see all model IDs for a race/slot:

```bash
uv run cexi gear json
...
- To re-import an edited mesh, use `cexi gear import`.

---

## Related commands

- **`cexi gear import`** — re-import an edited GLB back into the gear DAT
- **`cexi gear json`** — list all gear model IDs and their DAT paths
- **`cexi mesh export`** — same pipeline, for monster/NPC models
