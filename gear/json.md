...
```
uv run cexi gear json [RACE] [SLOT] [--search TEXT] [--output PATH]
uv run cexi gear search <query> [--race RACE] [--slot SLOT]
```
...
The old `cexi gear list` command remains as a hidden compatibility alias.
...
```
uv run cexi gear json --search main
```
...
`cexi gear json` expands all groups for all races, collects every unique
...
- **`cexi gear export`** — export a gear model to GLB/FBX for editing
- **`cexi gear import`** — re-import an edited GLB back into a gear DAT
- **`cexi ftable lookup`** — look up a single file_id to get its DAT path and header bytes
- **`cexi mesh export`** — equivalent for monster/NPC/entity models
