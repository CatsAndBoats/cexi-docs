...
> [`cexi dats`](../dats/README.md) and the hidden compatibility notes in
...
cexi gear edit HumeMale main 259 --hue 240 --hue-min 150 --hue-max 340 --saturation 30

# Red tint on gray metallic parts
cexi gear edit HumeMale main 259 --tint "#bb2222aa" --blend overlay --sat-max 0.15

# Full weapon hue shift
cexi gear edit HumeMale main 259 --hue 180

# Auto-detect race/slot/model from the DAT itself
cexi gear edit ROM/33/17 --hue 200
```
...
```bash
cexi gear edit HumeFemale body 34 --hue 200   # explicit
cexi gear edit ROM/33/17 --hue 200            # DAT path  (same model)
cexi gear edit 10578 --hue 200                # raw file_id (same model)
```
...
Or use `cexi gear json` to browse all registered gear models.
...
```bash
cexi gear edit HumeMale main 259 --hue 240 --hue-min 150 --hue-max 340
```
...
- The edit replaces the original model for ALL items that share the same
  ...
  action through [`cexi dats`](../dats/README.md); the old inject flow is hidden.
