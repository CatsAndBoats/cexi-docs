...
- A clean `ROM/1/41.DAT` (the first `cexi zone import` makes a pristine
  `ROM/1/41.DAT.base`; if `.base` already exists it is treated as the source of
  truth — make sure it's clean).
- `cexi` available (`uv run cexi …` or the installed entrypoint).
...
uv run cexi zone export ROM/1/41
...
uv run cexi zone import ROM/1/41 --prune
...
```sh
uv run cexi fx json ROM/1/41        # tki*/awa*/grid should be gone
```
