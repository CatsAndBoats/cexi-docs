...
> **Override order (CatsEyeXI):** the live client loads base-game ROM, then CatsEyeXI override
> DATs (`Ashita/polplugins/DATs/catseyexi/`), then ffxi-hd overrides. An override replaces the
...
* ROM10 = the custom content slot used by cexi tooling.
...
* Named at offset 0 by race/sex code + variant: `hm`/`hf` Hume M/F, `em`/`ef` Elvaan M/F,
  ...
  `lhm_` LOD). Same space as `cexi gear json`.
* List them: `cexi gear json`. Detail: [gear/json.md](gear/json.md)
...
* List them: `cexi model json`. Detail: [model/json.md](model/json.md),
...
* Unpack with `cexi ffximain` tools → `pol_decompressed.bin`/`.txt` for analysis.
