...
```
uv run cexi fx delete <dat> <name>... [--dry-run]
```
...
uv run cexi fx delete ROM/1/41 grid

# prefixes: removes tki1-5 + awa1-6 + grid (the whole fountain effect set)
uv run cexi fx delete ROM/1/41 tki awa grid

# preview only, write nothing
uv run cexi fx delete ROM/1/41 tki --dry-run
```
...
- A pristine **`<dat>.base`** backup is created (shared with `cexi zone import`).
  Restore the whole DAT from it — or run `cexi zone reset` — to undo.
...
top of mesh-merge/placement edits. But `cexi zone import` **rebuilds from `.base`**,
...
- **[`cexi fx json`](json.md)** — find the effect names to delete
- **[`cexi fx set`](set.md)** — edit effects instead of removing them
- **[`cexi fx copy`](copy.md)** — duplicate / transplant effects

---

# cexi fx delete-group

Delete an entire fixture group by **any member name** — the inverse of
[`cexi fx copy-group`](copy.md#cexi-fx-copy-group).
...
```
uv run cexi fx delete-group <dat> <seed> [--dry-run]
```
...
uv run cexi fx delete-group "ROM\1\41.DAT" xfd0 --dry-run

# delete the first torch group
uv run cexi fx delete-group "ROM\1\41.DAT" xfd0

# delete the second torch group (if a second copy-group was run)
uv run cexi fx delete-group "ROM\1\41.DAT" xfd1
```
...
- `cexi fx copy-group` prints the ready-to-paste delete command at the end of its output.
- Keeps a `<dat>.base` backup (shared with `cexi zone import` / `cexi fx delete`).

---

## Related commands

- **[`cexi fx copy-group`](copy.md#cexi-fx-copy-group)** — create the group in the first place
- **[`cexi fx delete`](delete.md)** — remove effects by name or prefix
