...
```
uv run cexi fx set <dat> <name>... [params]
```
...
|---|---|---|
| `--pos X Y Z` (alias `--at-pos`) | local position (`--at-pos` matches the `/cexi pos` output) | 3×f32 after the placed-mesh/texture ref |
...
- Edits are written to the DAT in place, with a `<dat>.base` backup of the
  pristine bytes. Restore from `.base` or `cexi zone reset` to undo.

### Caveat — ordering with `zone import`

`fx set` edits the DAT **in place** (it does not reset to `.base`), so it stacks on
mesh-merge/placement edits. But `cexi zone import` **rebuilds from `.base`** (which
...
- **[`cexi fx json`](json.md)** — see the current value of every param before editing
- **[`cexi fx copy`](copy.md)** — duplicate/transplant an effect (then `set --autorun`)
- **[`cexi fx delete`](delete.md)** — remove effects instead of editing them
