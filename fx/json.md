...
```
uv run cexi fx json <dat> [OPTIONS]
```
...
uv run cexi fx json ROM/1/41 --opcodes --output fountain_full.json
```
...
- Effect labels come from [`src/cexi/fx/fx_library.json`](../../src/cexi/fx/fx_library.json).
...
- `--opcodes` adds the full instruction stream decoded by [`src/cexi/fx/xi_opcodes.py`](../../src/cexi/fx/xi_opcodes.py).

---

## Related commands

- **[`cexi fx set`](set.md)** — edit the params this command decodes
- **[`cexi fx export`](export.md)** — export one effect's mesh, texture, and JSON
- **[`cexi fx delete`](delete.md)** — remove effects by exact name or prefix
