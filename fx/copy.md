...
```
uv run cexi fx copy <dat> <src> [params]
```
...
uv run cexi fx copy ROM/1/41 tki5 --offset 6 0 0
...
uv run cexi fx copy ROM/1/41 a133 --from ROM/0/73 --replace grid
```
...
- **[`cexi fx set`](set.md)** — after copying, reposition / recolour / `--autorun` it
- **[`cexi fx export`](export.md)** — preview an effect's mesh + texture before copying
- **[`cexi fx delete`](delete.md)** — remove an effect (or a botched copy)
...
```
uv run cexi fx copy-group <dat> <seed> [params]
```
...
uv run cexi fx copy-group "ROM\1\41.DAT" fd04 --from "ROM2\0\27.DAT" --pos -17.0274 -2.8627 6.7949

# second torch at a different position — auto-named xfd1, xfl1, ...
uv run cexi fx copy-group "ROM\1\41.DAT" fd04 --from "ROM2\0\27.DAT" --pos 5.0 -2.6 10.0
```
...
To delete: cexi fx delete-group "<output DAT path>" xfm0
```
...
- **Runtime effects** (`fr`/`fs` rim/spark roles, `builder: runtime`) have their
  ...
  `copy-group` patches these correctly; `cexi fx copy` alone would error with
...
- **`−Y` is UP** — same as `cexi fx copy`.

---

## Related commands

- **[`cexi fx delete-group`](delete.md#cexi-fx-delete-group)** — undo a group copy by any member name
- **[`cexi fx copy`](copy.md)** — single-effect copy with full options
- **[`cexi fx set`](set.md)** — reposition / recolour after copying
