...
> [effects.md](effects.md) (the `0x05` deep-dive + the `cexi fx` toolchain),
...
`cexi fx copy --from` automates (`_DEP_TYPES = 0x20, 0x21, 0x1F, 0x19, 0x2E`).
...
Full deep-dive (header fields, the 4 opcode sub-sections, the `cexi fx` param map) is
...
(`ParticleInitializers.kt:981-1057`). cexi reads/writes `0x20` via
[`cexi tex`](../dats/tex.md).
...
1. **Geometry** — author a `0x21` SpriteSheetMesh (billboard, for fire/glows) or `0x1F`
   ParticleMesh, with its `0x20` texture(s). (cexi: `cexi tex` round-trips textures;
...
3. **Generator** — a `0x05` ParticleGenerator: set attach type (`SourceActor`/
   ...
   Reuse an existing one as a template with `cexi fx copy`/`fx set`/`fx json --opcodes`.
