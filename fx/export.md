...
```
uv run cexi fx export <dat> <effect> [--out DIR]
```
...
uv run cexi fx export ROM/1/41 lt37 --out /tmp/lamp_light
```
...
- The `<effect>.json` is the same per-effect entry as [`cexi fx json`](json.md), and
...
- **[`cexi fx json`](json.md)** — inspect all effects' params as JSON
- **[`cexi fx copy`](copy.md)** — duplicate / transplant the effect instead of exporting
- **`cexi zone export`** — same GLB builder, for a whole zone (placed `0x1C` objects)

> Note: effect-placed meshes are often **orphan `0x2E` meshes** with no `0x1C`
> placement, so they never appear in `cexi zone export`'s GLB — `fx export` is how you
