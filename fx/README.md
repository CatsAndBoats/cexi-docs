...
glow, fire, smoke, bubbles, sky clouds, point lights. Mirrors the `cexi fx`
command group (`src/cexi/fx/`).
...
`cexi zone json --fx` provides the same effect dump from the zone command group.

## Typical workflows

```bash
# See what's in a zone, then dump the full param set for offline review
uv run cexi fx json ROM/1/41                     # JSON dump of effects + decoded params
...
- An effect references its mesh / textures / sub-effects **by 4-char FourCC**, never
  ...
  **invisible in `cexi zone export`** (this is why the fountain puddle never showed
  up in the GLB).
...
editable registry, [`src/cexi/fx/fx_library.json`](../../src/cexi/fx/fx_library.json).
...
- **[spells.md](spells.md)** — the **spell VFX** implementation reference: the verified
  ...
  `cexi.spell` module + `zone.spellList`/`zone.spellVfx` RPCs, the editor's Spells
...
- **[../dats/fx.md](../dats/fx.md)** — the original single-page `cexi fx` overview
