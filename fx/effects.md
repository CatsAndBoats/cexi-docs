...
> hands-on `0x05` deep-dive + the `cexi fx` toolchain.
...
all of which `cexi fx copy --from` now brings (`_DEP_TYPES` includes `0x21` and
...
exist only to be summoned by their effect. Since `cexi zone export` assembles only
...
```
cexi fx json   <dat>                 # every 0x05: name, size, params, placed mesh + position
cexi fx delete <dat> <name>...       # remove effect(s) by exact name or prefix
```
...
(same as mesh-merge growth). `--dry-run` previews. Implemented in `the src/cexi/fx/ package`.

Examples:
```
cexi fx json   ROM/1/41
cexi fx delete ROM/1/41 tki awa grid    # strip the whole fountain effect set
```
...
**Live position → DAT coords (Ashita).** The `cexitools` Ashita addon
(`addon/cexitools/`, `/cexi pos`) reports the player's position for
...
**Cross-DAT transplant.** `cexi fx copy --from <src_dat>` copies an effect between
...
1. **Delete** — ✅ DONE. `cexi fx delete`. Verified in-game: `tki`/`awa`/`grid`
...
- `src/cexi/zone/` — zone export/import (mesh `0x2E`, placement `0x1C`); no `0x05`
