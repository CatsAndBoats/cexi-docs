...
How a spell becomes moving particles, and how cexi loads + plays spell effects in the web
level editor. This is the **implementation reference** for the `cexi.spell` module, the editor's
...
| Table | Where | Maps |
|-------|-------|------|
| **SpellAnimationTable** | **server** data — LSB `sql/spell_list.sql` `animation` column. Carried in cexi as `src/cexi/spell/spell_anim_table.json` (928 entries, ported from the UE5 builtin fallback). | spellIndex → animationIndex |
...
> (`SpellTables.cpp:128`). In cexi: `parse_dmsg(data, bitmask=0)`. Using the default 0xFF garbles
...
> table. Both LSB and the UE5 reimpl carry it as server-sourced data. cexi ships the ported table so
...
python -c "import re,json; src=open('SpellAnimationTableData.cpp').read(); \
  ...
  json.dump(m, open('src/cexi/spell/spell_anim_table.json','w'), separators=(',',':'))"
```
...
command's absolute start frame = the sum of all `delay`s up to and including it. cexi flattens this to
...
## 3. The `cexi.spell` module

`src/cexi/spell/xi_spell.py` (+ data file `spell_anim_table.json`). All public functions are pure /
...
`src/cexi/zone/xi_bridge.py` (`handle_command`).
...
- Drag MIME `application/x-cexi-spell` (gated in `_isCexiDrag` + the canvas `drop` dispatch).
...
Backend (new module + RPCs) and frontend JS both change → restart `cexi gui zone` (kill orphaned
...
**cexi**:
- `src/cexi/spell/xi_spell.py`, `src/cexi/spell/spell_anim_table.json` — module + data.
- `src/cexi/zone/xi_bridge.py` — `zone.spellList`, `zone.spellVfx`.
- `src/cexi/event/xi_event.py` — `_scene_sections`, `_routine_sec2_commands` (reused decoders).
