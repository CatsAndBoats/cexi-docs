...
> The **`len` column is code-derived** from cexi's validated disassembler (`_FIXED_SIZES` /
> `_SUB_TABLES` in `src/cexi/event/xi_event.py`) and is byte-identical to the UE FFXIEngine
