# Authoring NPC dialogue events (`cexi event dialogue new`)
...
> **Status: shipped.** `cexi event dialogue new` + `cexi event dialogue actors`. Builds
...
cexi event dialogue actors 245                                   # find the NPC's server id
cexi event dialogue new 245 --json lines.json --actor 0x010F5022 # author the event → prints the id
```
...
> 2-byte selector, not a single byte. (cexi's decoder used to read a signed byte here and
...
### `cexi event dialogue new <zone|dat> --json <file> --actor <id> [flags]`
...
the CLI in [`src/cexi/event/xi_commands.py`](../../src/cexi/event/xi_commands.py)
(`dialogue_group`), the dialog codec in
[`src/cexi/dialog/xi_dialog.py`](../../src/cexi/dialog/xi_dialog.py).
...
references from "hundreds of `-1`" to **9 unresolved** — so `cexi event cutscene export` and the
...
- **One language table.** It edits the dialog DAT cexi resolves for the zone (typically the
