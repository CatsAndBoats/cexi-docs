...
> [`cexi event dialogue new`](authoring.md) appends lines and synthesizes a plain
...
build/extend resources
   ...
   └─ keep a <dat>.base backup (same convention as cexi zone / fx)
   ▼
...
Two integration modes, mirroring the rest of cexi:
...
- **Custom zone / DAT** — for a fresh zone, package the event set as that zone's DATs and
  register their file_ids (`cexi dats`, `cexi ftable`, `cexi zone new`).
...
- **Camera model — container now confirmed (2026-06-18), maths still open.** cexi has
