...
> bespoke to that engine** rather than retail. **Update (2026-06-18):** cexi has since
...
> **Confirmed (2026-06-18) — camera = `0x06` Route + `0x07` EffectRoutine.** cexi's
> `parse_camera_routes` (`src/cexi/event/xi_event.py`) byte-decoded a real cutscene's
...
`cexi event cutscene export 184` surfaces this directly — the `load_zone` lines read
...
## Inspecting a cutscene in cexi

`cexi event cutscene export <zone>` and the web level editor's **Timeline / Opcodes** views
...
> Decoders: `build_cutscene_timeline` + `parse_effect_routines` (`src/cexi/event/xi_event.py`),
> `build_character_glb` (`src/cexi/gear/xi_character.py`); served to the editor by
