...
Dumped via `cexi event cutscene export ROM/21/52.DAT --actor 0x010F3031 --all-events`.
...
`append_dialog_lines` strategy `cexi event dialogue new` already ships.
...
- **`qstc` semantics** — plausibly a server-completion signal, but its FourCC
  isn't in cexi's decode tables. Ship the compiler treating it as a plain
  scheduled task; verify against other quest events.

## Next actions

1. **Compiler skeleton** (`src/cexi/event/xi_compile.py`) — lower each step
...
4. **`cexi dats build` wiring** — teach the event action handler to accept
   `resources.cutscene`, parse it as `cexi.cutscene.v1`, and dispatch to
