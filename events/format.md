...
[`cexi ftable lookup`](../ftable/lookup.md) for its path. Event DATs carry the `evte`
...
- **`sceneSize` is the *unpadded* bytecode length** (it can be non-multiple-of-4). The block is
  ...
  cexi's writer (`xi_event.build_event_dat`, [authoring.md](authoring.md)) preserves this for a
