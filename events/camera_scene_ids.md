...
- Each **custom** cutscene needs its **own free mid-band file id** so cameras don't clobber each other. Allocator: `_camera_scene_fileid` in [`xi_bridge.py`](../../src/cexi/zone/xi_bridge.py).

## `datid_helper` tiers (HARD RULES for custom cameras)

```python
# cexi.event.xi_event._datid_helper
...
`cexi dats build`: the same `(ftval, vt)` is written at that file id into **both** the root
