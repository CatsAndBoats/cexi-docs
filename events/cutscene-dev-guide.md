...
A self-contained reference for picking up cutscene work in cexi. Assumes you know
...
**cexi decode chain** (all backend, Python):
...
**Parser**: `parse_look()` in `src/cexi/gear/xi_core.py`. Documented in
...
| File | Key functions |
|---|---|
| `src/cexi/event/xi_event.py` | `build_cutscene_timeline`, `parse_effect_routines`, `parse_routine_clip`, `parse_routine_motion`, `parse_pointlist`, `event_entity_positions`, `build_motion_index`, `derive_anim_base`, `parse_camera_routes` |
| `src/cexi/gear/xi_character.py` | `build_character_glb`, `_facing_axis`, `resolve_event_clips`, `_load_external_clips`, `_collect_clips` |
| `src/cexi/gear/xi_core.py` | `parse_look`, `resolve_gear_dat`, `race_skeleton_dat`, `model_file_id` |
| `src/cexi/zone/xi_bridge.py` | `_cutscene`, `_camera_scene_fileid`, `_write_camera_scene`, `_publish_pivot_tables`, `_scene_data` |
...
1. `cexi gui zone` (port 8777) — kills existing processes first or you get
...
5. The "Server Path" input (default `D:\cexi-server\catseyexi`) auto-resolves to the
   mission script and shows its `:event()` call.

**Kill/restart loop** (Windows):
```powershell
Stop-Process -Name "python" -Force  # or target the specific cexi gui zone PID
cexi gui zone
```
