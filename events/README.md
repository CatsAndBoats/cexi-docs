...
> **Status.** `cexi event cutscene export`, `cexi event dialogue new`, and `cexi event dialogue`
...
> `cexi event dialogue new` goes the other way — it **authors** a new NPC dialogue event
...
`src/cexi/zone/xi_inject.py`). For the base ROM
(`zone_id < 0x100`):
...
Resolve a file_id → DAT path with [`cexi ftable lookup`](../ftable/lookup.md). The
...
> file tables (it's how `cexi zone` resolves zones). The internal *byte* layout of the
...
- **[authoring.md](authoring.md)** — **`cexi event dialogue new`**: inject dialogue lines +
...
| `src/cexi/zone/xi_inject.py`, `xi_list.py` (`parse_dmsg`) | the per-zone file-id formulas + a working `d_msg` string parser |
