...
- `uv run cexi gear json` ([docs](gear/json.md)) — List all gear model entries across all races and slots
- `uv run cexi gear search <query>` — Search race/slot/model/file/DAT entries
- `uv run cexi gear export` / `uv run cexi gear import` — Existing gear edit round-trip
...
- `uv run cexi mount json` — enumerate mounts: id, EN/JP name, key item, file-id, ROM DAT, free/occupied
- `uv run cexi mount export <id>` — dump one mount's full record (names, help, key-item text) → JSON
- `uv run cexi mount import <id> --dat <file>` — override an existing mount's model
- `uv run cexi mount delete <id>` — remove a custom mount + emit server cleanup
- `uv run cexi dats prepare` / `cexi dats build` ([docs](dats/README.md)) — author brand-new mounts as reproducible package actions

#### References

- [mounts/README.md](mounts/README.md) — overview, `cexi mount` command table, EN/JP DAT-path reference
...
- `uv run cexi zone export` ([docs](zone/export.md)) — Decrypt + export a zone (meshes + placements + textures) to `.glb`/`.fbx`; `--collision` also exports the player-collision mesh
- `uv run cexi zone import` ([docs](zone/import.md)) — Import an edited GLB back (move/rotate/scale/delete + mesh-merge + add collision)
- `uv run cexi zone import-json` — Apply a JSON change-set from the web level editor to a local output mirror
- `uv run cexi dats prepare zone-changes.json ...` ([docs](dats/README.md)) — Add a zone change-set to a distributable DAT package
- `uv run cexi gui zone` — Serve the browser-based level editor at `http://localhost:8777/`
- `uv run cexi object …` ([docs](object/README.md)) — Add, move, replace, clone, or delete individual placements
- `uv run cexi zone navmesh` ([docs](zone/navmesh.md)) — Bake a server navmesh (`.nav`) from the zone's collision
- `uv run cexi zone new` — Create a blank zone from the pre-baked template; `--sky <DAT>` to splice atmosphere
- `uv run cexi zone delete <id>` — Remove a custom zone (400+), zero FTABLE10, print server SQL
- `uv run cexi zone build-from-manifest` — Assemble a zone from a Godot designer `build_manifest.json`
- `uv run cexi zone reset` — Restore a zone DAT to pristine
...
- `uv run cexi fx json` ([docs](fx/json.md)) — Dump every effect (+ decoded params, optionally raw opcodes) to JSON
- `uv run cexi fx set` ([docs](fx/set.md)) — Edit params in place: position / scale / color / draw-distance / spawn / count / autorun
- `uv run cexi fx copy` ([docs](fx/copy.md)) — Duplicate an effect — same-DAT, or cross-DAT (`--from`) bringing its deps
- `uv run cexi fx delete` ([docs](fx/delete.md)) — Remove effect(s) by exact name or name-prefix
- `uv run cexi fx export` ([docs](fx/export.md)) — Export an effect's 3D mesh + material/texture + decoded params bundle
- `uv run cexi gui weapon` ([docs](fx/editor.md)) — Serve the browser-based particle/weapon-effect editor at `http://localhost:8776/`
- `uv run cexi gui spells` ([docs](fx/spells.md)) — Serve the browser-based spell-effect editor at `http://localhost:8774/`
...
prompts, and scripted scene action. The `cexi event` commands act on the per-zone
Event DAT — `cexi event cutscene export` (bytecode disassembly) and `cexi event
...
- `uv run cexi audio json/search --type music` ([docs](audio/README.md)) — List/search music, with titles from MusicInfo
- `uv run cexi audio json/search --type sfx` ([docs](audio/README.md)) — List/search sound effects, with game categories
- `uv run cexi audio export --type music|sfx` ([docs](audio/README.md)) — Decode music or sound effects
- `uv run cexi audio decode FILE…` ([docs](audio/README.md)) — Decode explicit `.bgw`/`.spw` paths to `.wav`
- `uv run cexi audio info FILE` ([docs](audio/README.md)) — Dump a file's parsed header
- `uv run cexi audio refs <dat>` ([docs](audio/refs.md)) — List the sounds a DAT references (spell VFX, zone ambient, mob) → JSON
- `uv run cexi batch audio_music` / `audio_sfx` ([docs](audio/README.md)) — Decode **all** music / sfx + write a categorised `catalog.json`
...
`cexi event dialogue` is the single home for NPC-text work. Authoring (`actors`,
...
- `uv run cexi event dialogue actors <DAT>` ([docs](events/authoring.md)) — list a zone's NPC actor ids (pick one for `new --actor`)
- `uv run cexi event dialogue new <DAT> --json lines.json --actor <id>` ([docs](events/authoring.md)) — append lines + splice a new event that prints them
- `uv run cexi event dialogue info <DAT>` ([docs](dialog/export.md)) — entry count + opcode histogram
- `uv run cexi event dialogue search <DAT> "text"` ([docs](dialog/export.md)) — find the index + entry for some text (substring/`--regex`, spans line breaks) — pairs with `edit --index`
- `uv run cexi event dialogue export <DAT>` ([docs](dialog/export.md)) — decode to 3 sibling files under `exports/event/dialogue/<rom>/`: `<stem>.json` (text), `.opcodes.json`, `.hex.json` (`-o` base, `--no-opcodes`/`--no-raw`, `--json` stdout, `--preview`, `--grep`, `--prompts-only`)
- `uv run cexi event dialogue edit <DAT> --index <N> --text "<text>"` ([docs](dialog/edit.md)) — author custom dialog: `\n` newline, `\v` press-enter ▼, `{player}`/`{npc}`/`{auto:N}`; rebuilds the DAT to the output mirror (`--dry-run`)
- `uv run cexi event dialogue reset <DAT>` ([docs](dialog/edit.md)) — undo all edits: delete the output mirror, or restore from `<dat>.base` in-place (`--dry-run`); `--full` also resets the zone's **event** DAT, fully undoing a `dialogue new`
...
- `uv run cexi ffximain text-dump` ([docs](reference/ffximain.md)) — Write `pol_decompressed.bin` + full disassembly `.txt` (~45 MB, ~2–3 min)
- `uv run cexi ffximain unpack` ([docs](reference/ffximain.md)) — Write `FFXiMain_unpacked.dll` as a valid PE (load in Ghidra/IDA, image base `0x10000000`)
...
- `pol_decompressed.bin` / `pol_decompressed.txt` — outputs of `uv run cexi ffximain text-dump`
...
- `uv run cexi utils dds2png` ([docs](utils/texture.md)) — Convert `DXT1` / `DXT3` / `DXT5` DDS files to PNG
- `uv run cexi utils png2dds` ([docs](utils/texture.md)) — Convert PNG files to DDS using `texconv`
