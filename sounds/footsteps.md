...
reimplementation (`thirdparty/xim/`) and cross-checked against cexi's collision
decoder (`src/cexi/zone/xi_collision.py`).
...
> decode in [`xi_collision.py`](../../src/cexi/zone/xi_collision.py). This doc is
...
cexi decodes the identical expression per-triangle in
[`xi_collision.py:238`](../../src/cexi/zone/xi_collision.py) and emits the result
as Wavefront `usemtl col_floor_<terrain>` / `col_wall_<terrain>` material groups
(`TERRAIN_NAMES`, [`xi_collision.py:95`](../../src/cexi/zone/xi_collision.py);
`material_name()`, [`xi_collision.py:125`](../../src/cexi/zone/xi_collision.py)),
...
cexi has a targeted helper for the sound-pointer half:

```bash
uv run cexi zone footsteps ROM10/2/4 --from ROM/0/57 --terrain snow
```
...
## Cross-reference: cexi tooling

cexi already round-trips the terrain types that drive all of the above:

- Decode/export per-triangle terrain as obj material groups —
  [`xi_collision.py`](../../src/cexi/zone/xi_collision.py) (`TERRAIN_NAMES`,
  `_parse_mesh`, `material_name`).
- The exported `.collision.json` sidecar records a one-line footstep note
  ([`xi_collision.py:484`](../../src/cexi/zone/xi_collision.py)).
