...
A field guide to crashes the FFXI client (Ashita) throws after a cexi publish, and how
...
[`xi_bridge.py`](../src/cexi/zone/xi_bridge.py). If you see churn again, check that the
...
**Fix.** `clone_zone_companions` in [`xi_inject.py`](../src/cexi/zone/xi_inject.py) copies
